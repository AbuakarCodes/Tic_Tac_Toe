...
function WinningAlgorithm(player) {
  for (const ComboArry_element of combosArry) {
    if (ComboArry_element.every((C) => player.Coordinates.includes(C))) return { winnigPlayerID: player.id }
  }
...
export { Buttons_Disable, Bot_Pattren_Genrator, WinningAlgorithm, Change_BotImg }
